	...
	*/
	protected void doPost(HttpServletRequest request, HttpServletResponse response)	throws ServletException, IOException {
		...
		if (request.getParameter("consultarCliente") != null) {
			...
			Clientes cDTO = cDAO.buscar_cliente(cedulaCliente);
			if (cDTO != null) {
				String nombreCliente;
				cedulaCliente = cDTO.getCedula_cliente();
				nombreCliente = cDTO.getNombre_cliente();
				response.sendRedirect(
						"views/Ventas/regVenta.jsp?cedulaCliente=" + cedulaCliente + "&&nombreCliente=" + nombreCliente);
			} else {
				JOptionPane.showMessageDialog(null, "Error al consultar el cliente");
				response.sendRedirect("views/Ventas/regVenta.jsp");
			}
		}
		...
		if (request.getParameter("consultarPd1") != null) {
			...
			if (pdDTO != null) {
				...
				ivaPd1 = pdDTO.getIva();
				response.sendRedirect("views/Ventas/regVenta.jsp?codigo1=" + codigo1 + "&&nombrePd1=" + nombrePd1 + "&&precioPd1="
						+ precioPd1 + "&&ivaPd1=" + ivaPd1);
			} else {
				response.sendRedirect("views/Ventas/regVenta.jsp?mensaje=Error al consultar el producto");
		...
		if (request.getParameter("consultarPd2") != null) {
			...
			if (pdDTO != null) {
				...
				ivaPd2 = pdDTO.getIva();
				response.sendRedirect("views/Ventas/regVenta.jsp?codigo2=" + codigo2 + "&&nombrePd2=" + nombrePd2 + "&&precioPd2="
						+ precioPd2 + "&&ivaPd2=" + ivaPd2);
			} else {
				response.sendRedirect("views/Ventas/regVenta.jsp?mensaje=Error al consultar el producto");
			}
		}
		...
		if (request.getParameter("consultarPd3") != null) {
			...
			if (pdDTO != null) {
				...
				ivaPd3 = pdDTO.getIva();
				response.sendRedirect("views/Ventas/regVenta.jsp?codigo3=" + codigo3 + "&&nombrePd3=" + nombrePd3 + "&&precioPd3="
						+ precioPd3 + "&&ivaPd3=" + ivaPd3);
			} else {
				response.sendRedirect("views/Ventas/regVenta.jsp?mensaje=Error al consultar el producto");
			}
		}
		...
		if (request.getParameter("confirmar") != null) {
			...
			response.sendRedirect("views/Ventas/regVenta.jsp?mensaje=Venta y detalle insertados exitosamente" + "&&cant1=" + cant1
					+ "&&valorT1=" + valorT1 + "&&cant2=" + cant2 + "&&valorT2=" + valorT2 + "&&cant3=" + cant3
					+ "&&valorT3=" + valorT3 + "&&totalVenta=" + totalVenta + "&&totalIva=" + totalIva + "&&totalPagar="
					+ totalPagar + "&&codigoV=" + codigoV);
		}

		if (request.getParameter("limpiar") != null) {
			response.sendRedirect("views/Ventas/regVenta.jsp?men=Datos eliminados!");
		}
